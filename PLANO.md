...
Última atualização: 2026-08-31 (cadastro+workspaces+geo no ar e verificados)
...
| 105 | **Cadastro e convite destravados.** (a) `sign_up` do Supabase estourava `email rate limit exceeded` no SMTP compartilhado e ainda exigia clicar num link — a conta agora nasce confirmada via **API de admin** (zero email) e o backend loga em seguida pra devolver a sessão; domínio de teste também passa. (b) Convite por email de quem ainda não tem conta dava 500 (`42P10`: upsert mirando índice parcial `lower()`) — agora checa e insere; o convite pendente vira membro de verdade no `_bootstrap_workspace` quando a pessoa se cadastra. Verificado em prod ponta a ponta: cadastro→sessão, código errado→403, convite→`pending:true` na lista, convidado se cadastra→vira membro (`memberCount` sobe). `smoke_test.py` **53/53**. | `backend/app/routers/auth.py`, `backend/app/routers/workspaces.py` |
| 104 | **Geo do Ao Vivo verificado + fixes soltos.** Migration 008 confirmada (colunas `geo_*` em `live_beats`); heartbeat com IP BR em produção resolveu `São Caetano do Sul/SP` e `GET /api/live/geo` devolveu o ponto — mapa do Brasil funcional ponta a ponta. `notify_sale` (push de venda) agora vai a **todos os membros do workspace**, não só ao criador do funil (cai no dono no modo legado). `DashboardPage`: o `RadarChart` de VSL só renderiza com 3+ VSLs — recharts emitia `<path d="Z">` (erro no console do navegador) com 0-2 pontos; verificado em prod: 0 paths inválidos no DOM. | `backend/app/services/push.py`, `frontend/src/pages/DashboardPage.tsx` |
...
| 106 | **Puxador de print funcionava, mas voltava página em branco / "Page Not Found (PNX003)"** em sites atrás de Cloudflare/Akamai (serasa, bancos, gov). Causa: o Chromium headless entregava `navigator.webdriver=true` e UA `HeadlessChrome`. `_via_playwright` agora: `--disable-blink-features=AutomationControlled`, `new_context` com UA de Chrome real + `locale=pt-BR` + timezone SP, e espera `networkidle` (6s, degrada) antes do print. Também: `_store` cria o bucket `screenshots` (público) sozinho com a service key se não existir, em vez de erro vermelho. Verificado local: `https://www.serasa.com.br/` renderiza a home real (antes: branco). | `backend/app/services/screenshot.py` |
